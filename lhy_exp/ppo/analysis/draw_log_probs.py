...
def analyze_pkl_files(folder_path="./async_10/gen_samples"):
    # 收集所有数据
    all_data = []
    ...
    for filename in tqdm(pkl_files, desc="Processing files"):
        ...
        for old_seq, cur_seq, cur_log_prob in zip(data["old_output_ids"], data["cur_output_ids"], data["cur_log_prob"]):
            ...
            if cur_seq_len > 20000:
                print(cur_seq_len, cur_log_prob)
            if cur_seq_len < 100:
                print(cur_seq_len, cur_log_prob)
            '''

        old_log_prob = data["old_log_prob"].sum(dim=1) / torch.tensor(old_lengths)
        cur_log_prob = data["cur_log_prob"].sum(dim=1) / torch.tensor(cur_lengths)


        # 记录当前step的数据
        all_data.append({
            "step": int(filename.split("_")[1].split(".")[0]),
            "old_log_prob": old_log_prob.tolist(),
            "cur_log_prob": cur_log_prob.tolist(),
...
def visualize_results(all_data):
    ...
    for data in all_data:
        ...
        # 计算新旧log_prob的均值
        avg_old_log_prob.append(np.mean(data["old_log_prob"]))
        avg_cur_log_prob.append(np.mean(data["cur_log_prob"]))
        
        # 按长度分组旧数据
        for length, log_prob in zip(data["old_lengths"], data["old_log_prob"]):
            if length not in old_length_log_prob:
                old_length_log_prob[length] = []
            old_length_log_prob[length].append(log_prob)
        
        # 按长度分组新数据
        for length, log_prob in zip(data["cur_lengths"], data["cur_log_prob"]):
    ...
    for data in all_data:
        # 旧数据散点（蓝色）
        plt.scatter([data["step"]]*len(data["old_log_prob"]), 
                    data["old_log_prob"],
                    alpha=0.3, c='blue', s=10, 
                    label='Old' if data == all_data[0] else None)
        # 新数据散点（橙色）
        plt.scatter([data["step"]]*len(data["cur_log_prob"]), 
                    data["cur_log_prob"],
                    alpha=0.3, c='orange', s=10, 
                    label='Current' if data == all_data[0] else None)
    
    # 绘制均值折线
    plt.plot(steps, avg_old_log_prob, 'b-', linewidth=2, label='Old Average')
    plt.plot(steps, avg_cur_log_prob, 'r-', linewidth=2, label='Current Average')
    plt.xlabel("Training Step")
    plt.ylabel("Log Prob")
    plt.title("Log Prob vs Training Step")
    plt.grid(True)
    plt.legend()
    ...
    plt.ylabel("Log Prob")
    plt.title("Log Prob vs Sequence Length")
    plt.grid(True)
    plt.legend()
    
    plt.tight_layout()
    plt.savefig("log_prob_async_10.png", dpi=400)
    plt.close()
