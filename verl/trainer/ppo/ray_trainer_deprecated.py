...
def _torch_profiler(
    ...
    参数：
    file_prefix : 可选的文件名前缀（例如"resnet50_exp1"）
    device : 要分析的设备（默认cuda:0）
    activities : 要监控的活动列表（默认监控CPU和CUDA）
    profiler_kwargs : 传递给torch.profiler.profile的额外参数
    """
    trace_dir = os.path.abspath("./trace")
    ...
    prof_args = {
        ...
        "schedule": schedule(wait=1, warmup=0, active=1, repeat=1),
        "record_shapes": True,
        "profile_memory": True,
        "with_stack": True,
    ...
    def _trace_handler(prof: profile):
        ...
        # 构造最终文件名前缀
        final_prefix = f"{host_name}_{timestamp}"
        if file_prefix:
        ...
        try:
            prof.export_memory_timeline(
                f"{trace_dir}/{final_prefix}_memory.html", 
                device=device
            )
...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        with _torch_profiler(file_prefix="lhy_exp_profile") as prof:
                ...
                for batch_dict in self.train_dataloader:
                    ...
                    with _timer("step", timing_raw):
                        ...
                        with _timer("gen", timing_raw):
                            with record_function("## Actor Gen ##"):
                        ...
                        with _timer("old_log_prob", timing_raw):
                            with record_function("## Actor Infer ##"):
                        ...
                        if self.use_reference_policy:
                            ...
                            with _timer("ref", timing_raw):
                                with record_function("## Ref Infer ##"):
                        ...
                        if self.use_critic:
                            with _timer("values", timing_raw):
                                with record_function("## Critic Infer ##"):
                        ...
                        with _timer("adv", timing_raw):
                            ...
                            if self.use_rm:
                                ...
                                with record_function("## Reward Infer ##"):
                        ...
                        if self.use_critic:
                            with _timer("update_critic", timing_raw):
                                with record_function("## Critic Train ##"):
                        ...
                        if self.config.trainer.critic_warmup <= self.global_steps:
                            ...
                            with _timer("update_actor", timing_raw):
                                with record_function("## Actor Train ##"):
