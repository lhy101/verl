        ...
        with _torch_profiler(file_prefix="update_policy") as prof:
            ...
            with record_function("## load actor param and optimizer ##"):
                ...
                if self._is_offload_param:
                    ...
                    log_gpu_memory_usage("After load actor model during update_actor", logger=logger)
                if self._is_offload_optimizer:
    ...
    def update_critic(self, data: DataProto):
        with _torch_profiler(file_prefix="update_critic") as prof:
            ...
            with record_function("## load critic param and optimizer ##"):
                if self._is_offload_param:
                    load_fsdp_model_to_gpu(self.critic_module)
                    log_gpu_memory_usage("After load critic model during update_critic", logger=logger)
                if self._is_offload_optimizer:
                    load_fsdp_optimizer(optimizer=self.critic_optimizer, device_id=torch.cuda.current_device())
                    log_gpu_memory_usage("After load critic optimizer during update_critic", logger=logger)
            ...
            with record_function("## offload actor param and optimizer ##"):
                if self._is_offload_param:
                    offload_fsdp_model_to_cpu(self.critic_module)
                    log_gpu_memory_usage("After offload critic model during update_critic", logger=logger)
                if self._is_offload_optimizer:
                    offload_fsdp_optimizer(optimizer=self.critic_optimizer)
                    log_gpu_memory_usage("After offload critic optimizer during update_critic", logger=logger)
